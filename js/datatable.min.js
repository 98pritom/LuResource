...
class DataTable {
  ...
  checkId() {
    (this.el.hasAttribute("id") && 0 != this.el.id.length) ||
  ...
  getPgMessage() {
    ...
    document.querySelector(`#${this.el.id}_pgdisplay`).innerHTML =
      this.locale.SHOWING_TO.replace("{FROM}", e)
        .replace("{TO}", t)
  ...
  getData() {
    let e = [],
      t = [];
    return (
      this.el.querySelectorAll("thead th").forEach((e) => t.push(e.innerHTML)),
      this.el.querySelectorAll("tbody > tr").forEach((i) => {
        let a = {};
        i.querySelectorAll("td").forEach((e, i) => (a[t[i]] = e.innerHTML)),
          e.push(a);
      }),
      e
    );
  ...
  search(e) {
    if (
      ...
      i.setAttribute("aria-live", "polite"),
        (i.innerHTML = this.locale.NO_RESULT),
        (i.style.textAlign = "center"),
        (i.colSpan = e.length),
        t.appendChild(i),
        (this.el.querySelector("tbody").innerHTML = ""),
        this.el.querySelector("tbody").appendChild(t);
  ...
  pagination() {
    ...
    if (this.currPage > 1) {
      ...
      t.setAttribute("aria-controls", this.el.id),
        t.setAttribute("aria-label", this.locale.GOTO_FIRST),
        (t.innerHTML = "&#10094;&#10094;"),
        t.classList.add("btn-char"),
        (t.onclick = () => this.changePage(1)),
        e.appendChild(t);
      let i = document.createElement("button");
      i.setAttribute("aria-controls", this.el.id),
        i.setAttribute("aria-label", this.locale.GOTO_PREV),
        (i.innerHTML = "&#10094;"),
        i.classList.add("btn-char"),
        (i.onclick = () => this.changePage(this.currPage - 1)),
        e.appendChild(i);
    }
    for (let i = this.currPage - 2; i < this.currPage + 2; i++) {
      if (i < 1 || i > t) continue;
      let a = document.createElement("button");
      a.setAttribute("aria-controls", this.el.id),
        a.setAttribute("aria-label", this.locale.GOTO_PAGE.replace("{NUM}", i)),
        a.classList.add("btn"),
        (a.innerHTML = i),
        i == this.currPage
    ...
    if (this.currPage < t) {
      let i = document.createElement("button");
      i.setAttribute("aria-controls", this.el.id),
        i.setAttribute("aria-label", this.locale.GOTO_NEXT),
        (i.innerHTML = "&#10095;"),
        i.classList.add("btn-char"),
        (i.onclick = () => this.changePage(this.currPage + 1)),
        e.appendChild(i);
      let a = document.createElement("button");
      a.setAttribute("aria-controls", this.el.id),
        a.setAttribute("aria-label", this.locale.GOTO_LAST),
        (a.innerHTML = "&#10095;&#10095;"),
        a.classList.add("btn-char"),
        (a.onclick = () => this.changePage(t)),
        e.appendChild(a);
    }
    (document.querySelector(`#${this.el.id}_pagination`).innerHTML = ""),
      document.querySelector(`#${this.el.id}_pagination`).appendChild(e);
  }
  changeOrder(e, t) {
    let i = this.perPage * (this.currPage - 1),
      a =
        this.filtered.length > this.perPage
          ? i + this.perPage
  ...
  toggleHead(e, t) {
    ...
    e.forEach((e) => {
      e.removeAttribute("aria-sort"),
        e.setAttribute("aria-label", `${e.innerText}: non attivo`);
    }),
      0 == i.length || "d" == i[0]
        ? (t.setAttribute("aria-sort", "ascending"),
          t.setAttribute(
            "aria-label",
            `${t.innerText}: ${this.locale.ASC_ACTIVE}`
          ),
          this.changeOrder(t.innerText, 1))
        : (t.setAttribute("aria-sort", "descending"),
          t.setAttribute(
            "aria-label",
            `${t.innerText}: ${this.locale.DESC_ACTIVE}`
          ),
          this.changeOrder(t.innerText, -1));
  }
  init() {
    ...
    for (let e = 1; e <= 5; e++)
      a.innerHTML += `<option ${1 == e ? "selected" : ""} value='${
        10 * e
      }'>\n                    ${10 * e}\n                </option>`;
    if (
      ((a.onchange = () => {
      ...
      i.setAttribute("aria-controls", this.el.id),
        i.classList.add(`${DT_PREFIX}__search`),
        i.setAttribute("role", "searchbox"),
        (i.placeholder = this.locale.SEARCH),
        i.classList.add("control"),
        (i.type = "search"),
        i.addEventListener("input", () => this.search(i.value)),
        e.appendChild(i),
        t.appendChild(e);
    }
    e.appendChild(t);
    ...
    r.forEach((e) => {
      e.setAttribute("tabindex", 0),
        ...
        e.setAttribute("aria-controls", this.el.id),
        e.setAttribute(
          "aria-label",
          `${e.innerText}: ${this.locale.NOT_ACTIVE}`
        ),
        e.addEventListener("click", () => this.toggleHead(r, e)),
    ...
    s.classList.add(`${DT_PREFIX}__table`),
    ...
    c.setAttribute("aria-live", "polite"),
      (c.id = `${this.el.id}_pgdisplay`),
      n.appendChild(c);
    let d = document.createElement("div");
    (d.id = `${this.el.id}_pagination`),
      n.appendChild(d),
      e.appendChild(n),
      this.pagination(),
      this.getPgMessage(),
